...
func applyAttributes(src *sysl.Statement, dst *sysl.Statement) bool {
	...
	for _, stmt := range stmts {
		applied = applyAttributes(src, stmt)
	}
	return applied
}
...
func checkCalls(mod *sysl.Module, appname string, epname string, dst *sysl.Statement) bool {
	...
	case *sysl.Statement_Call:
		...
		if app == nil {
			fmt.Printf("%s::%s calls non-existant App: %s", appname, epname, s.Call.Target.Part)
			return false
		}
		_, valid = app.Endpoints[s.Call.Endpoint]
		if !valid {
			fmt.Printf("%s::%s calls non-existant App <- Endpoint (%s <- %s)", appname, epname, s.Call.Target.Part, s.Call.Endpoint)
		}
		return valid
	case *sysl.Statement_Action:
		return true
				...
				case *sysl.Statement_Call:
					...
					for call_epname, call_endpoint := range app.Endpoints {
						...
						for _, call_stmt := range call_endpoint.Stmt {
							applied = applyAttributes(collector_stmt, call_stmt)
						}
						if !applied {
							fmt.Printf("App::Endpoint (%s::%s) calls non-existant endpoint (%s <- %s)\n", appName, call_epname, x.Call.Target.Part, x.Call.Endpoint)
						}
					}
				default:
